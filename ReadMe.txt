========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : SetupAutoTrader
========================================================================


AppWizard が生成した SetupAutoTrader には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには SetupAutoTrader を構成している各ファイルの概要説明が含まれています。
...
SetupAutoTrader.h
    ...
    みます） をインクルードし、また CSetupAutoTraderApp アプリケーションクラスを宣
    言します。

SetupAutoTrader.cpp
    このファイルは CSetupAutoTraderApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\SetupAutoTrader.ico
    このファイルは、アプリケーションのアイコンとして使用されるアイコンファイ
    ルです。このアイコンはリソース ファイル SetupAutoTrader.rc によってインクルード
    されます。

res\SetupAutoTrader.rc2
...
SetupAutoTraderDlg.h, SetupAutoTraderDlg.cpp - ダイアログ
    これらのファイルは CSetupAutoTraderDlg クラスを含みます。このクラスは 
    アプリケーションのメイン ダイアログの動作を定義します。ダイアログ
    テンプレートは Microsoft Visual C++ で編集可能な SetupAutoTrader.rc に含
    まれます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) SetupAutoTrader.pch 
